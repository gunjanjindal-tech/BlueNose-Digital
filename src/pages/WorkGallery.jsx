 ...
 <div className="absolute inset-0 pointer-events-none overflow-hidden">
  <div className="absolute top-9 left-0 flex w-max animate-brandSlide">
    {[...Array(2)].map((_, groupIndex) => (
      ...
      <h2 className="text-center text-4xl md:text-5xl 2xl:text-6xl font-extrabold text-[#063349]">
        Work <span className="text-[#0E6388]">Gallery</span>
      </h2>

      <p className="text-center text-gray-600  mx-auto mt-4 mb-16 xl:text-3xl">
        Campaigns that performed. Strategies that scaled. Brands that grew.
      </p>

      {/* Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 max-w-7xl mx-auto 2xl:max-w-none 2xl:px-32">
        {clients.map((c, i) => (
...
function WorkCard({ data }) {
  ...
  return (
    ...
    >
      ...
      {/* NAME */}
      <h3
        className="text-2xl font-bold text-center 2xl:text-4xl 2xl:pt-5"
      ...
      {/* DESC */}
      <p
        className="text-center text-sm mt-1 mb-5 2xl:text-xl"
        style={{ color: dark ? "#fff" : "#063349" }}
      >
        {data.desc}
      </p>
      ...
      {/* STATS */}
      <div className="grid grid-cols-2 gap-3 2xl:text-xl">
        {data.stats.map((s, i) => (
          <div
            key={i}
